...
`POST /api/backup/preview` accepts `backup` plus optional `location_updates`, an array of at most 100 complete saved connection records (`id`, `name`, `host`, `username`, `transport`, `port`, `fingerprint`). IDs must exist in the backup and cannot repeat. Passwords and other fields are rejected. Omit the array for an unchanged legacy restore. Validation uses the existing IPv4/transport/certificate rules and makes no router calls or disk writes.

The preview returns `files`, `replace`, `groups`, normalized `locations`, `connection_changes` (location ID/label, field, before, after) and `destination` (data directory and boolean environment-configuration presence). These booleans are not credential validation. No secret values are returned.

`POST /api/backup/restore` accepts the same backup/updates plus `confirmation: "RESTORE"`; nonempty location updates also require `migration_ack: true`. The router must be disconnected. The backend validates again and restores the transformed bundle through the existing recovery workflow, returning `restored`, `recovery`, `locations` and `connection_changes`. Location IDs, archive scopes and ledger rows are preserved. Clients should require a fresh preview after every edit; the browser rejects preview responses if inputs changed while awaiting them. No accounts or router configuration are restored.
